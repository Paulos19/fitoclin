...
import { AnamnesisRequestButtons } from "@/components/dashboard/anamnesis-request-buttons";
      ...
      <Tabs defaultValue="anamnesis" className="w-full">
        <div className="w-full overflow-x-auto pb-2">
          <TabsList className="bg-[#0A311D] border border-[#2A5432] p-1 rounded-xl w-full md:w-max flex justify-start mb-2">
            ...
            <TabsTrigger value="post-consultation" className={tabTriggerStyle}><MessageCircle className="w-4 h-4 mr-1" /> Pós-Atendimento</TabsTrigger>
        ...
        {/* 1. PRÉ-ATENDIMENTO (ANAMNESE) */}
        <TabsContent value="anamnesis" className="animate-in slide-in-from-bottom-2 space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#0A311D]/30 p-4 rounded-xl border border-[#2A5432]/30">
            <div>
              <h3 className="text-white font-semibold flex items-center gap-2">
                <Clock className="w-4 h-4 text-[#76A771]" /> Ações de Anamnese
              </h3>
              <p className="text-gray-400 text-xs mt-1">Solicite o preenchimento ou libere o acesso manual para o paciente</p>
            </div>
            <AnamnesisRequestButtons
              patientId={patient.id}
              patientEmail={patient.user.email}
              patientName={patient.user.name}
              patientPhone={patient.phone || ""}
              isLocked={patient.allowAnamnesisUpdate}
            />
          </div>
          ...
          {patient.anamnesis ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center bg-[#0A311D]/20 rounded-2xl border-2 border-dashed border-[#2A5432]/50">
              <div className="h-20 w-20 rounded-full bg-[#2A5432]/20 flex items-center justify-center mb-6">
                <FileText className="h-10 w-10 text-gray-500" />
              </div>
              <h3 className="text-xl font-bold text-white mb-2">Anamnese não preenchida</h3>
              <p className="text-gray-400 max-w-sm">
                O paciente ainda não preencheu o formulário pré-atendimento. Use o painel acima para solicitar via WhatsApp ou E-mail.
              </p>
            </div>
          )}
        </TabsContent>
